...
import {
  ...
  decideRouting,
  runClassifierAttempt,
  resolveAvailableTier,
  extractTextFromContent,
  hasImageAttachment,
} from './routing';
import { runSystemOneClassifier } from './typesafe';
...
export const registerRouterProvider = (
  ...
  const modelDefinitions = profileList.map((name) => {
    ...
    if (hasReasoning) {
      ...
      if (profileLevels.has('max' as ThinkingLevel)) map.max = 'max';
    ...
    streamSimple(
      ...
      (async () => {
        try {
          ...
          // result would be downgraded anyway, saving an unnecessary classifier call.
          if (
            (state.currentConfig.classifier ||
            ...
            let classifiedBySystemOne = false;

            if (state.currentConfig.classifier) {
              const systemOneResult = await runSystemOneClassifier(
                state.currentConfig.classifier,
                context,
                state.lastDecision?.phase,
                { signal: options?.signal },
              );
              if (!systemOneResult.ok) {
                if (systemOneResult.reason === 'request-cancelled') {
                  throw new RoutingCancelledError();
                }
                classifierFailure = systemOneResult.reason;
              } else {
                const tier = resolveAvailableTier(
                  profile,
                  systemOneResult.classification.tier,
                );
                const resolvedTierNote =
                  tier === systemOneResult.classification.tier
                    ? tier
                    : `${systemOneResult.classification.tier} resolved to ${tier}`;
                decision = buildRoutingDecision(
                  model.id,
                  profile,
                  tier,
                  phaseForTier(tier),
                  `TypeSafe Jev classifier: ${resolvedTierNote} (confidence ${systemOneResult.classification.confidence}).`,
                  state.thinkingByProfile[model.id],
                  true,
                );
                classifiedBySystemOne = true;
              }
            }

            if (!classifiedBySystemOne && state.currentConfig.classifierModel) {
              const classifierResult = await runClassifierAttempt(
              ...
              if (!classifierResult.ok) {
                if (classifierResult.reason === 'request-cancelled') {
                  throw new RoutingCancelledError();
                }
                llmFailure = classifierResult.reason;
              } else {
                const source = classifierFailure
                  ? ` after TypeSafe ${classifierFailure}`
                  : '';
                decision = buildRoutingDecision(
                  model.id,
                  profile,
                  classifierResult.classification.tier,
                  phaseForTier(classifierResult.classification.tier),
                  `LLM classifier${source}: ${classifierResult.classification.reasoning}`,
                  state.thinkingByProfile[model.id],
                  true,
                );
              }
            ...
            if (!classifiedBySystemOne && (classifierFailure || llmFailure)) {
              const failures = [
                classifierFailure ? `TypeSafe ${classifierFailure}` : undefined,
                llmFailure ? `LLM ${llmFailure}` : undefined,
              ].filter((failure): failure is string => Boolean(failure));
              decision = {
                ...decision,
                reasoning: `${decision.reasoning} (Classifier fallback: ${failures.join('; ')}.)`,
              };
            }
          }
          ...
          if (isStaleCtx || error instanceof RoutingCancelledError) {
            stream.push({
              type: 'done',
              reason: 'stop',
