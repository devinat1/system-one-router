...
describe('config.ts', () => {
  ...
  describe('mergeConfig', () => {
    ...
    it('should replace or explicitly disable the inherited TypeSafe classifier', () => {
      const base: RouterConfig = {
        classifier: {
          provider: 'typesafe',
          type: 'system-one',
      ...
      const replacement = mergeConfig(base, {
        classifier: {
          provider: 'typesafe',
          type: 'system-one',
          model: 'jev-preview',
      ...
      expect(replacement.classifier).toEqual({
        provider: 'typesafe',
        type: 'system-one',
        model: 'jev-preview',
        timeoutMs: 1200,
      ...
      const disabled = mergeConfig(base, { classifier: null });
      expect(disabled.classifier).toBeUndefined();
    });
  });
  ...
  describe('normalizeConfig – classifier config variants', () => {
    ...
    it('should normalize TypeSafe defaults and sanitize invalid settings', () => {
      const base = {
        profiles: { balanced: { high: { model: 'openai/gpt-4o' } } },
      };
      const { config, warnings } = normalizeConfig({
        ...base,
        classifier: { provider: 'typesafe', type: 'system-one' },
      ...
      const invalid = normalizeConfig({
        ...base,
        classifier: { provider: 'typesafe', type: 'system-one', timeoutMs: 0 },
      } as unknown as RouterConfig);
      expect(invalid.config.classifier).toBeUndefined();
      expect(invalid.warnings).toEqual([
        'Ignored invalid TypeSafe classifier configuration.',
      ]);
    });
  });
});
