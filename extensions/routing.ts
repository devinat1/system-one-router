...
export type LlmClassifierFailure =
  | 'model-unavailable'
...
export type LlmClassifierAttempt =
  | { ok: true; classification: { tier: RouterTier; reasoning: string } }
  | { ok: false; reason: LlmClassifierFailure };

export const runClassifierAttempt = async (
  ...
  signal?: AbortSignal,
  timeoutMs = 10_000,
): Promise<LlmClassifierAttempt> => {
  if (signal?.aborted) return { ok: false, reason: 'request-cancelled' };
  ...
  try {
    ...
    if (!auth.ok || !auth.apiKey) {
      return { ok: false, reason: 'authentication-unavailable' };
    }
...
${currentPhase === 'implementation' ? 'Consider that the conversation is currently in an implementation phase. Bias toward "medium" unless the request is clearly planning or a simple summary.' : ''}`;
    ...
    if (signal?.aborted) return { ok: false, reason: 'request-cancelled' };
    if (timedOut) return { ok: false, reason: 'request-timeout' };
...
export const runClassifier = async (
  ...
  return result.ok ? result.classification : undefined;
};
